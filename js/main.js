...
// global object.
if(typeof module!=="undefined"&&module.exports){module.exports=_;_._=_}else{
...
_.map=function(obj,iterator,context){var results=[];if(obj==null)return results;if(nativeMap&&obj.map===nativeMap)return obj.map(iterator,context);each(obj,function(value,index,list){results[results.length]=iterator.call(context,value,index,list)});return results};
...
_.find=_.detect=function(obj,iterator,context){var result;any(obj,function(value,index,list){if(iterator.call(context,value,index,list)){result=value;return true}});return result};
...
// Aliased as `all`.
_.every=_.all=function(obj,iterator,context){var result=true;if(obj==null)return result;if(nativeEvery&&obj.every===nativeEvery)return obj.every(iterator,context);each(obj,function(value,index,list){if(!(result=result&&iterator.call(context,value,index,list)))return breaker});return result};
// Determine if at least one element in the object matches a truth test.
// Delegates to **ECMAScript 5**'s native `some` if available.
// Aliased as `any`.
var any=_.some=_.any=function(obj,iterator,context){iterator=iterator||_.identity;var result=false;if(obj==null)return result;if(nativeSome&&obj.some===nativeSome)return obj.some(iterator,context);each(obj,function(value,index,list){if(result|=iterator.call(context,value,index,list))return breaker});return!!result};
// Determine if a given value is included in the array or object using `===`.
// Aliased as `contains`.
_.include=_.contains=function(obj,target){var found=false;if(obj==null)return found;if(nativeIndexOf&&obj.indexOf===nativeIndexOf)return obj.indexOf(target)!=-1;any(obj,function(value){if(found=value===target)return true});return found};
// Invoke a method (with arguments) on every item in a collection.
_.invoke=function(obj,method){var args=slice.call(arguments,2);return _.map(obj,function(value){return(method.call?method||value:value[method]).apply(value,args)})};
...
_.sortBy=function(obj,iterator,context){return _.pluck(_.map(obj,function(value,index,list){return{value:value,criteria:iterator.call(context,value,index,list)}}).sort(function(left,right){var a=left.criteria,b=right.criteria;return a<b?-1:a>b?1:0}),"value")};
// Groups the object's values by a criterion produced by an iterator
_.groupBy=function(obj,iterator){var result={};each(obj,function(value,index){var key=iterator(value,index);(result[key]||(result[key]=[])).push(value)});return result};
// Use a comparator function to figure out at what index an object should
// be inserted so as to maintain order. Uses binary search.
_.sortedIndex=function(array,obj,iterator){iterator||(iterator=_.identity);var low=0,high=array.length;while(low<high){var mid=low+high>>1;iterator(array[mid])<iterator(obj)?low=mid+1:high=mid}return low};
...
_.flatten=function(array){return _.reduce(array,function(memo,value){if(_.isArray(value))return memo.concat(_.flatten(value));memo[memo.length]=value;return memo},[])};
// Return a version of the array that does not contain the specified value(s).
_.without=function(array){return _.difference(array,slice.call(arguments,1))};
// Produce a duplicate-free version of the array. If the array has already
// been sorted, you have the option of using a faster algorithm.
// Aliased as `unique`.
_.uniq=_.unique=function(array,isSorted){return _.reduce(array,function(memo,el,i){if(0==i||(isSorted===true?_.last(memo)!=el:!_.include(memo,el)))memo[memo.length]=el;return memo},[])};
...
_.indexOf=function(array,item,isSorted){if(array==null)return-1;var i,l;if(isSorted){i=_.sortedIndex(array,item);return array[i]===item?i:-1}if(nativeIndexOf&&array.indexOf===nativeIndexOf)return array.indexOf(item);for(i=0,l=array.length;i<l;i++)if(array[i]===item)return i;return-1};
// Delegates to **ECMAScript 5**'s native `lastIndexOf` if available.
_.lastIndexOf=function(array,item){if(array==null)return-1;if(nativeLastIndexOf&&array.lastIndexOf===nativeLastIndexOf)return array.lastIndexOf(item);var i=array.length;while(i--)if(array[i]===item)return i;return-1};
// Generate an integer Array containing an arithmetic progression. A port of
// the native Python `range()` function. See
// [the Python documentation](http://docs.python.org/library/functions.html#range).
_.range=function(start,stop,step){if(arguments.length<=1){stop=start||0;start=0}step=arguments[2]||1;var len=Math.max(Math.ceil((stop-start)/step),0);var idx=0;var range=new Array(len);while(idx<len){range[idx++]=start;start+=step}return range};
...
_.bind=function(func,obj){if(func.bind===nativeBind&&nativeBind)return nativeBind.apply(func,slice.call(arguments,1));var args=slice.call(arguments,2);return function(){return func.apply(obj,args.concat(slice.call(arguments)))}};
// Bind all of an object's methods to that object. Useful for ensuring that
// all callbacks defined on an object belong to it.
_.bindAll=function(obj){var funcs=slice.call(arguments,1);if(funcs.length==0)funcs=_.functions(obj);each(funcs,function(f){obj[f]=_.bind(obj[f],obj)});return obj};
// Memoize an expensive function by storing its results.
_.memoize=function(func,hasher){var memo={};hasher||(hasher=_.identity);return function(){var key=hasher.apply(this,arguments);return hasOwnProperty.call(memo,key)?memo[key]:memo[key]=func.apply(this,arguments)}};
// Delays a function for the given number of milliseconds, and then calls
// it with the arguments supplied.
_.delay=function(func,wait){var args=slice.call(arguments,2);return setTimeout(function(){return func.apply(func,args)},wait)};
// Defers a function, scheduling it to run after the current call stack has
// cleared.
_.defer=function(func){return _.delay.apply(_,[func,1].concat(slice.call(arguments,1)))};
// Internal function used to implement `_.throttle` and `_.debounce`.
var limit=function(func,wait,debounce){var timeout;return function(){var context=this,args=arguments;var throttler=function(){timeout=null;func.apply(context,args)};if(debounce)clearTimeout(timeout);if(debounce||!timeout)timeout=setTimeout(throttler,wait)}};
...
_.wrap=function(func,wrapper){return function(){var args=[func].concat(slice.call(arguments));return wrapper.apply(this,args)}};
// Returns a function that is the composition of a list of functions, each
// consuming the return value of the function that follows.
_.compose=function(){var funcs=slice.call(arguments);return function(){var args=slice.call(arguments);for(var i=funcs.length-1;i>=0;i--){args=[funcs[i].apply(this,args)]}return args[0]}};
...
_.extend=function(obj){each(slice.call(arguments,1),function(source){for(var prop in source){if(source[prop]!==void 0)obj[prop]=source[prop]}});return obj};
...
if(a===b)return true;
// Different types?
var atype=typeof a,btype=typeof b;if(atype!=btype)return false;
...
for(var key in a)if(!(key in b)||!_.isEqual(a[key],b[key]))return false;return true};
// Is a given array or object empty?
_.isEmpty=function(obj){if(_.isArray(obj)||_.isString(obj))return obj.length===0;for(var key in obj)if(hasOwnProperty.call(obj,key))return false;return true};
...
var addToWrapper=function(name,func){wrapper.prototype[name]=function(){var args=slice.call(arguments);unshift.call(args,this._wrapped);return result(func.apply(_,args),this._chain)}};
// Add all of the Underscore functions to the wrapper object.
_.mixin(_);
// Add all mutator Array functions to the wrapper.
each(["pop","push","reverse","shift","sort","splice","unshift"],function(name){var method=ArrayProto[name];wrapper.prototype[name]=function(){method.apply(this._wrapped,arguments);return result(this._wrapped,this._chain)}});
// Add all accessor Array functions to the wrapper.
each(["concat","join","slice"],function(name){var method=ArrayProto[name];wrapper.prototype[name]=function(){return result(method.apply(this._wrapped,arguments),this._chain)}});
...
if(customEvents[type]){if(customEvents[type].condition)fn=customHandler(element,fn,type,customEvents[type].condition,true);type=customEvents[type].base||type}entry=registry.put(new RegEntry(element,type,fn,originalFn,namespaces[0]&&namespaces));entry.handler=entry.isNative?nativeHandler(element,entry.handler,args):customHandler(element,entry.handler,type,false,args,false);if(entry.eventSupport)listener(entry.target,entry.eventType,entry.handler,true,entry.customType)},del=function(selector,fn,$){return function(e){var target,i,array=typeof selector==="string"?$(selector,this):selector;for(target=e.target;target&&target!==this;target=target.parentNode){for(i=array.length;i--;){if(array[i]===target){return fn.apply(target,arguments)}}}}},remove=function(element,typeSpec,fn){var k,m,type,namespaces,i,rm=removeListener,isString=typeSpec&&typeof typeSpec==="string";if(isString&&typeSpec.indexOf(" ")>0){
// remove(el, 't1 t2 t3', fn) or remove(el, 't1 t2 t3')
typeSpec=typeSpec.split(" ");for(i=typeSpec.length;i--;)remove(element,typeSpec[i],fn);return element}type=isString&&typeSpec.replace(nameRegex,"");if(type&&customEvents[type])type=customEvents[type].type;if(!typeSpec||isString){
// remove(el) or remove(el, t1.ns) or remove(el, .ns) or remove(el, .ns1.ns2.ns3)
if(namespaces=isString&&typeSpec.replace(namespaceRegex,""))namespaces=namespaces.split(".");rm(element,type,fn,namespaces)}else if(typeof typeSpec==="function"){
// remove(el, fn)
rm(element,null,typeSpec)}else{
// remove(el, { t1: fn1, t2, fn2 })
for(k in typeSpec){if(typeSpec.hasOwnProperty(k))remove(element,k,typeSpec[k])}}return element},add=function(element,events,fn,delfn,$){var type,types,i,args,originalFn=fn,isDel=fn&&typeof fn==="string";if(events&&!fn&&typeof events==="object"){for(type in events){if(events.hasOwnProperty(type))add.apply(this,[element,type,events[type]])}}else{args=arguments.length>3?slice.call(arguments,3):[];types=(isDel?fn:events).split(" ");isDel&&(fn=del(events,originalFn=delfn,$))&&(args=slice.call(args,1));
// special case for one()
this===ONE&&(fn=once(remove,element,events,fn,originalFn));for(i=types.length;i--;)addListener(element,types[i],fn,originalFn,args)}return element},one=function(){return add.apply(ONE,arguments)},fireListener=W3C_MODEL?function(isNative,type,element){var evt=doc.createEvent(isNative?"HTMLEvents":"UIEvents");evt[isNative?"initEvent":"initUIEvent"](type,true,true,win,1);element.dispatchEvent(evt)}:function(isNative,type,element){element=targetElement(element,isNative);
// if not-native then we're using onpropertychange so we just increment a custom property
isNative?element.fireEvent("on"+type,doc.createEventObject()):element["_on"+type]++},fire=function(element,type,args){var i,j,l,names,handlers,types=type.split(" ");for(i=types.length;i--;){type=types[i].replace(nameRegex,"");if(names=types[i].replace(namespaceRegex,""))names=names.split(".");if(!names&&!args&&element[eventSupport]){fireListener(nativeEvents[type],type,element)}else{
// non-native event, either because of a namespace, arguments or a non DOM element
// iterate over all listeners and manually 'fire'
handlers=registry.get(element,type);args=[false].concat(args);for(j=0,l=handlers.length;j<l;j++){if(handlers[j].inNamespaces(names))handlers[j].handler.apply(element,args)}}}return element},clone=function(element,from,type){var i=0,handlers=registry.get(from,type),l=handlers.length;for(;i<l;i++)handlers[i].original&&add(element,handlers[i].type,handlers[i].original);return element},bean={add:add,one:one,remove:remove,clone:clone,fire:fire,noConflict:function(){context[name]=old;return this}};if(win[attachEvent]){
// for IE, clean up on unload to avoid leaks
var cleanup=function(){var i,entries=registry.entries();for(i in entries){if(entries[i].type&&entries[i].type!=="unload")remove(entries[i].element,entries[i].type)}win[detachEvent]("onunload",cleanup);win.CollectGarbage&&win.CollectGarbage()};win[attachEvent]("onunload",cleanup)}return bean});/**
...
draw:function(el,data,options,GraphKlass){GraphKlass=GraphKlass||Flotr.Graph;return new GraphKlass(el,data,options)},/**
   ...
   */
merge:function(src,dest){var i,v,result=dest||{};for(i in src){v=src[i];if(v&&typeof v==="object"){if(v.constructor===Array){result[i]=this._.clone(v)}else if(v.constructor!==RegExp&&!this._.isElement(v)){result[i]=Flotr.merge(v,dest?dest[i]:undefined)}else{result[i]=v}}else{result[i]=v}}return result},/**
...
base:Math.E,titleAlign:"center",margin:true},x2axis:{},yaxis:{ticks:null,// => format: either [1, 3] or [[1, 'a'], 3]
minorTicks:null,// => format: either [1, 3] or [[1, 'a'], 3]
showLabels:true,// => setting to true will show the axis ticks labels, hide otherwise
showMinorLabels:false,// => true to show the axis minor ticks labels, false to hide
labelsAngle:0,// => labels' angle, in degrees
...
scaling:"linear",// => Scaling, can be 'linear' or 'logarithmic'
base:Math.E,titleAlign:"center",margin:true},y2axis:{titleAngle:270},grid:{color:"#545454",// => primary color used for outline and labels
...
circular:false},mouse:{track:false,// => true to track the mouse, no tracking otherwise
trackAll:false,position:"se",// => position of the value box (default south-east)
...
create:function(tag){return document.createElement(tag)},node:function(html){var div=Flotr.DOM.create("div"),n;div.innerHTML=html;n=div.children[0];div.innerHTML="";return n},/**
   * Remove all children.
   */
empty:function(element){element.innerHTML=""},hide:function(element){Flotr.DOM.setStyles(element,{display:"none"})},/**
   * Insert a child.
   * @param {Element} element
   * @param {Element|String} Element or string to be appended.
   */
insert:function(element,child){if(_.isString(child))element.innerHTML+=child;else if(_.isElement(child))element.appendChild(child)},
// @TODO find xbrowser implementation
opacity:function(element,opacity){element.style.opacity=opacity},position:function(element,p){if(!element.offsetParent)return{left:element.offsetLeft||0,top:element.offsetTop||0};p=this.position(element.offsetParent);p.left+=element.offsetLeft;p.top+=element.offsetTop;return p},removeClass:function(element,name){var classList=element.className?element.className:"";element.className=_.filter(classList.split(/\s+/g),function(c){if(c!=name)return true}).join(" ")},setStyles:function(element,o){_.each(o,function(value,key){element.style[key]=value})},show:function(element){Flotr.DOM.setStyles(element,{display:""})},/**
   * Return element size.
   */
size:function(element){return{height:element.offsetHeight,width:element.offsetWidth}}}})();/**
 * Flotr Event Adapter
 */
(function(){var F=Flotr,bean=F.bean;F.EventAdapter={observe:function(object,name,callback){bean.add(object,name,callback);return this},fire:function(object,name,args){bean.fire(object,name,args);if(typeof Prototype!="undefined")Event.fire(object,name,args);
...
_observe:observe,processColor:function(color,options){var o={x1:0,y1:0,x2:this.plotWidth,y2:this.plotHeight,opacity:1,ctx:this.ctx};_.extend(o,options);return flotr.Color.processColor(color,o)},/**
   ...
   */
findDataRanges:function(){var a=this.axes,xaxis,yaxis,range;_.each(this.series,function(series){range=series.getRange();if(range){xaxis=series.xaxis;yaxis=series.yaxis;xaxis.datamin=Math.min(range.xmin,xaxis.datamin);xaxis.datamax=Math.max(range.xmax,xaxis.datamax);yaxis.datamin=Math.min(range.ymin,yaxis.datamin);yaxis.datamax=Math.max(range.ymax,yaxis.datamax);xaxis.used=xaxis.used||range.xused;yaxis.used=yaxis.used||range.yused}},this);
...
element:this.el,data:series.data,color:series.color,shadowSize:series.shadowSize,xScale:xaxis.d2p,yScale:yaxis.d2p,xInverse:xaxis.p2d,yInverse:yaxis.p2d};options=flotr.merge(type,options);
// Fill
...
mouseDownHandler:function(event){/*
    ...
    */
if(this.mouseUpHandler)return;this.mouseUpHandler=_.bind(function(e){E.stopObserving(document,"mouseup",this.mouseUpHandler);E.stopObserving(document,"mousemove",this.mouseDownMoveHandler);this.mouseDownMoveHandler=null;this.mouseUpHandler=null;
// @TODO why?
//e.stop();
...
size={};size.width=el.clientWidth;size.height=el.clientHeight;if(size.width<=0||size.height<=0||o.resolution<=0){throw"Invalid dimensions for plot, width = "+size.width+", height = "+size.height+", resolution = "+o.resolution}
...
var context=canvas.getContext("2d");if(!window.G_vmlCanvasManager)context.scale(o.resolution,o.resolution);return context}},_initPlugins:function(){
...
_.each(flotr.plugins,function(plugin,name){_.each(plugin.callbacks,function(fn,c){this.observe(this.el,c,_.bind(fn,this))},this);this[name]=flotr.clone(plugin);_.each(this[name],function(fn,p){if(_.isFunction(fn))this[name][p]=_.bind(fn,this)},this)},this)},/**
   ...
   */
_initOptions:function(opts){var options=flotr.clone(flotr.defaultOptions);options.x2axis=_.extend(_.clone(options.xaxis),options.x2axis);options.y2axis=_.extend(_.clone(options.yaxis),options.y2axis);this.options=flotr.merge(opts||{},options);if(this.options.grid.minorVerticalLines===null&&this.options.xaxis.scaling==="logarithmic"){this.options.grid.minorVerticalLines=true}if(this.options.grid.minorHorizontalLines===null&&this.options.yaxis.scaling==="logarithmic"){this.options.grid.minorHorizontalLines=true}E.fire(this.el,"flotr:afterinitoptions",[this]);this.axes=flotr.Axis.getAxes(this.options);
// Initialize some variables used throughout this function.
var assignedColors=[],colors=[],ln=this.series.length,neededColors=this.series.length,oc=this.options.colors,usedColors=[],variation=0,c,i,j,s;
// Collect user-defined colors from series.
for(i=neededColors-1;i>-1;--i){c=this.series[i].color;if(c){--neededColors;if(_.isNumber(c))assignedColors.push(c);else usedColors.push(flotr.Color.parse(c))}}
// Calculate the number of colors that need to be generated.
for(i=assignedColors.length-1;i>-1;--i)neededColors=Math.max(neededColors,assignedColors[i]+1);
// Generate needed number of colors.
for(i=0;colors.length<neededColors;){c=oc.length==i?new flotr.Color(100,100,100):flotr.Color.parse(oc[i]);
// Make sure each serie gets a different color.
var sign=variation%2==1?-1:1,factor=1+sign*Math.ceil(variation/2)*.2;c.scale(factor,factor,factor);/**
       * @todo if we're getting too close to something else, we should probably skip this one
       */
colors.push(c);if(++i>=oc.length){i=0;++variation}}
// Fill the options with the generated colors.
for(i=0,j=0;i<ln;++i){s=this.series[i];
// Assign the color.
if(!s.color){s.color=colors[j++].toString()}else if(_.isNumber(s.color)){s.color=colors[s.color].toString()}
// Every series needs an axis
if(!s.xaxis)s.xaxis=this.axes.x;if(s.xaxis==1)s.xaxis=this.axes.x;else if(s.xaxis==2)s.xaxis=this.axes.x2;if(!s.yaxis)s.yaxis=this.axes.y;if(s.yaxis==1)s.yaxis=this.axes.y;else if(s.yaxis==2)s.yaxis=this.axes.y2;
 ...
 * Flotr Axis Library
 */
(function(){var _=Flotr._,LOGARITHMIC="logarithmic";function Axis(o){this.orientation=1;this.offset=0;this.datamin=Number.MAX_VALUE;this.datamax=-Number.MAX_VALUE;_.extend(this,o)}
// Prototype
Axis.prototype={setScale:function(){var length=this.length,max=this.max,min=this.min,offset=this.offset,orientation=this.orientation,options=this.options,logarithmic=options.scaling===LOGARITHMIC,scale;if(logarithmic){scale=length/(log(max,options.base)-log(min,options.base))}else{scale=length/(max-min)}this.scale=scale;
// Logarithmic?
if(logarithmic){this.d2p=function(dataValue){return offset+orientation*(log(dataValue,options.base)-log(min,options.base))*scale};this.p2d=function(pointValue){return exp((offset+orientation*pointValue)/scale+log(min,options.base),options.base)}}else{this.d2p=function(dataValue){return offset+orientation*(dataValue-min)*scale};this.p2d=function(pointValue){return(offset+orientation*pointValue)/scale+min}}},calculateTicks:function(){var options=this.options;this.ticks=[];this.minorTicks=[];
// User Ticks
if(options.ticks){this._cleanUserTicks(options.ticks,this.ticks);this._cleanUserTicks(options.minorTicks||[],this.minorTicks)}else{if(options.mode=="time"){this._calculateTimeTicks()}else if(options.scaling==="logarithmic"){this._calculateLogTicks()}else{this._calculateTicks()}}
// Ticks to strings
_.each(this.ticks,function(tick){tick.label+=""});_.each(this.minorTicks,function(tick){tick.label+=""})},/**
   * Calculates the range of an axis to apply autoscaling.
   */
calculateRange:function(){if(!this.used)return;var axis=this,o=axis.options,min=o.min!==null?o.min:axis.datamin,max=o.max!==null?o.max:axis.datamax,margin=o.autoscaleMargin;if(o.scaling=="logarithmic"){if(min<=0)min=axis.datamin;
// Let it widen later on
if(max<=0)max=min}if(max==min){var widen=max?.01:1;if(o.min===null)min-=widen;if(o.max===null)max+=widen}if(o.scaling==="logarithmic"){if(min<0)min=max/o.base;// Could be the result of widening
var maxexp=Math.log(max);if(o.base!=Math.E)maxexp/=Math.log(o.base);maxexp=Math.ceil(maxexp);var minexp=Math.log(min);if(o.base!=Math.E)minexp/=Math.log(o.base);minexp=Math.ceil(minexp);axis.tickSize=Flotr.getTickSize(o.noTicks,minexp,maxexp,o.tickDecimals===null?0:o.tickDecimals);
...
decimals,minorTickSize,v,v2,i,j;if(o.minorTickFreq)minorTickSize=tickSize/o.minorTickFreq;
// Then store all possible ticks.
for(i=0;(v=v2=start+i*tickSize)<=max;++i){
// Round (this is always needed to fix numerical instability).
decimals=o.tickDecimals;if(decimals===null)decimals=1-Math.floor(Math.log(tickSize)/Math.LN10);if(decimals<0)decimals=0;v=v.toFixed(decimals);axis.ticks.push({v:v,label:o.tickFormatter(v,{min:axis.min,max:axis.max})});if(o.minorTickFreq){for(j=0;j<o.minorTickFreq&&i*tickSize+j*minorTickSize<max;++j){v=v2+j*minorTickSize;axis.minorTicks.push({v:v,label:o.tickFormatter(v,{min:axis.min,max:axis.max})})}}}}};
// Static Methods
_.extend(Axis,{getAxes:function(options){return{x:new Axis({options:options.xaxis,n:1,length:this.plotWidth}),x2:new Axis({options:options.x2axis,n:2,length:this.plotWidth}),y:new Axis({options:options.yaxis,n:1,length:this.plotHeight,offset:this.plotHeight,orientation:-1}),y2:new Axis({options:options.y2axis,n:2,length:this.plotHeight,offset:this.plotHeight,orientation:-1})}}});
// Helper Methods
...
topPadding:.1,// => top padding in percent
grouped:false},stack:{positive:[],negative:[],_positive:[],// Shadow
_negative:[]},draw:function(options){var context=options.context;this.current+=1;context.save();context.lineJoin="miter";
...
// Stacked bars
if(stack){stackValue=yValue>0?stack.positive:stack.negative;stackOffset=stackValue[xValue]||stackOffset;stackValue[xValue]=stackOffset+yValue}left=xScale(xValue-bisection);right=xScale(xValue+barWidth-bisection);top=yScale(yValue+stackOffset);bottom=yScale(stackOffset);
...
updateSelection:function(){if(!this.lastMousePos.pageX)return;this.selection.selecting=true;if(this.multitouches){this.selection.setSelectionPos(this.selection.selection.first,this.getEventPosition(this.multitouches[0]));this.selection.setSelectionPos(this.selection.selection.second,this.getEventPosition(this.multitouches[1]))}else if(this.options.selection.pinchOnly){return}else{this.selection.setSelectionPos(this.selection.selection.second,this.lastMousePos)}this.selection.clearSelection();if(this.selection.selectionIsSane()){this.selection.drawSelection()}},/**
   ...
   */
clearSelection:function(){if(!this.selection.prevSelection)return;var prevSelection=this.selection.prevSelection,lw=1,plotOffset=this.plotOffset,x=Math.min(prevSelection.first.x,prevSelection.second.x),y=Math.min(prevSelection.first.y,prevSelection.second.y),w=Math.abs(prevSelection.second.x-prevSelection.first.x),h=Math.abs(prevSelection.second.y-prevSelection.first.y);this.octx.clearRect(x+plotOffset.left-lw+.5,y+plotOffset.top-lw,w+2*lw+.5,h+2*lw+.5);this.selection.prevSelection=null},/**
   * Determines whether or not the selection is sane and should be drawn.
   * @return {Boolean} - True when sane, false otherwise.
   */
selectionIsSane:function(){var s=this.selection.selection;return Math.abs(s.second.x-s.first.x)>=5||Math.abs(s.second.y-s.first.y)>=5}})})();(function(){var D=Flotr.DOM,_=Flotr._;Flotr.addPlugin("legend",{options:{show:true,// => setting to true will show the legend, hide otherwise
...
backgroundOpacity:.85},callbacks:{"flotr:afterinit":function(){this.legend.insertLegend()}},/**
   * Adds a legend div to the canvas container or draws it on the canvas.
   */
insertLegend:function(){if(!this.options.legend.show)return;var series=this.series,plotOffset=this.plotOffset,options=this.options,legend=options.legend,fragments=[],rowStarted=false,ctx=this.ctx,itemCount=_.filter(series,function(s){return s.label&&!s.hide}).length,p=legend.position,m=legend.margin,opacity=legend.backgroundOpacity,i,label,color;if(itemCount){var lbw=legend.labelBoxWidth,lbh=legend.labelBoxHeight,lbm=legend.labelBoxMargin,offsetX=plotOffset.left+m,offsetY=plotOffset.top+m,labelMaxWidth=0,style={size:options.fontSize*1.1,color:options.grid.color};
// We calculate the labels' max width
for(i=series.length-1;i>-1;--i){if(!series[i].label||series[i].hide)continue;label=legend.labelFormatter(series[i].label);labelMaxWidth=Math.max(labelMaxWidth,this._text.measureText(label,style).width)}var legendWidth=Math.round(lbw+lbm*3+labelMaxWidth),legendHeight=Math.round(itemCount*(lbm+lbh)+lbm);
// Default Opacity
if(!opacity&&opacity!==0){opacity=.1}if(!options.HtmlText&&this.textEnabled&&!legend.container){if(p.charAt(0)=="s")offsetY=plotOffset.top+this.plotHeight-(m+legendHeight);if(p.charAt(0)=="c")offsetY=plotOffset.top+this.plotHeight/2-(m+legendHeight/2);if(p.charAt(1)=="e")offsetX=plotOffset.left+this.plotWidth-(m+legendWidth);
// Legend box
color=this.processColor(legend.backgroundColor,{opacity:opacity});ctx.fillStyle=color;ctx.fillRect(offsetX,offsetY,legendWidth,legendHeight);ctx.strokeStyle=legend.labelBoxBorderColor;ctx.strokeRect(Flotr.toPixel(offsetX),Flotr.toPixel(offsetY),legendWidth,legendHeight);
// Legend labels
var x=offsetX+lbm;var y=offsetY+lbm;for(i=0;i<series.length;i++){if(!series[i].label||series[i].hide)continue;label=legend.labelFormatter(series[i].label);ctx.fillStyle=series[i].color;ctx.fillRect(x,y,lbw-1,lbh-1);ctx.strokeStyle=legend.labelBoxBorderColor;ctx.lineWidth=1;ctx.strokeRect(Math.ceil(x)-1.5,Math.ceil(y)-1.5,lbw+2,lbh+2);
// Legend text
...
executeOnType:function(s,method,args){var success=false,options;if(!_.isArray(s))s=[s];function e(s,index){_.each(_.keys(flotr.graphTypes),function(type){if(s[type]&&s[type].show&&this[type][method]){options=this.getOptions(s,type);options.fill=!!s.mouse.fillColor;options.fillStyle=this.processColor(s.mouse.fillColor||"#ffffff",{opacity:s.mouse.fillOpacity});options.color=s.mouse.lineColor;options.context=this.octx;options.index=index;if(args)options.args=args;this[type][method].call(this[type],options);success=true}},this)}_.each(s,e,this);return success},/**
   ...
   */
drawHit:function(n){var octx=this.octx,s=n.series;if(s.mouse.lineColor){octx.save();octx.lineWidth=s.points?s.points.lineWidth:1;octx.strokeStyle=s.mouse.lineColor;octx.fillStyle=this.processColor(s.mouse.fillColor||"#ffffff",{opacity:s.mouse.fillOpacity});octx.translate(this.plotOffset.left,this.plotOffset.top);if(!this.hit.executeOnType(s,"drawHit",n)){var xa=n.xaxis,ya=n.yaxis;octx.beginPath();
...
octx.arc(xa.d2p(n.x),ya.d2p(n.y),s.points.hitRadius||s.points.radius||s.mouse.radius,0,2*Math.PI,true);octx.fill();octx.stroke();octx.closePath()}octx.restore();this.clip(octx)}this.prevHit=n},/**
   ...
   */
clearHit:function(){var prev=this.prevHit,octx=this.octx,plotOffset=this.plotOffset;octx.save();octx.translate(plotOffset.left,plotOffset.top);if(prev){if(!this.hit.executeOnType(prev.series,"clearHit",this.prevHit)){
// TODO fix this (points) should move to general testable graph mixin
...
"</div>","</div>","</td>",'<td class="flotr-legend-label">',label,"</td>")}if(rowStarted)fragments.push("</tr>");if(fragments.length>0){var table='<table style="font-size:smaller;color:'+options.grid.color+'">'+fragments.join("")+"</table>";if(legend.container){D.empty(legend.container);D.insert(legend.container,table)}else{var styles={position:"absolute",zIndex:"2",border:"1px solid "+legend.labelBoxBorderColor};if(p.charAt(0)=="n"){styles.top=m+plotOffset.top+"px";styles.bottom="auto"}else if(p.charAt(0)=="c"){styles.top=m+(this.plotHeight-legendHeight)/2+"px";styles.bottom="auto"}else if(p.charAt(0)=="s"){styles.bottom=m+plotOffset.bottom+"px";styles.top="auto"}if(p.charAt(1)=="e"){styles.right=m+plotOffset.right+"px";styles.left="auto"}else if(p.charAt(1)=="w"){styles.left=m+plotOffset.left+"px";styles.right="auto"}var div=D.create("div"),size;div.className="flotr-legend";D.setStyles(div,styles);D.insert(div,table);D.insert(this.el,div);if(!opacity)return;var c=legend.backgroundColor||options.grid.backgroundColor||"#ffffff";_.extend(styles,D.size(div),{backgroundColor:c,zIndex:"",border:""});styles.width+="px";styles.height+="px";
// Put in the transparent background separately to avoid blended labels and
div=D.create("div");div.className="flotr-legend-bg";D.setStyles(div,styles);D.opacity(div,opacity);D.insert(div," ");D.insert(this.el,div)}}}}}})})();(function(){var D=Flotr.DOM;Flotr.addPlugin("titles",{callbacks:{"flotr:afterdraw":function(){this.titles.drawTitles()}},/**
   * Draws the title and the subtitle
   */
drawTitles:function(){var html,options=this.options,margin=options.grid.labelMargin,ctx=this.ctx,a=this.axes;if(!options.HtmlText&&this.textEnabled){var style={size:options.fontSize,color:options.grid.color,textAlign:"center"};
// Add subtitle
if(options.subtitle){Flotr.drawText(ctx,options.subtitle,this.plotOffset.left+this.plotWidth/2,this.titleHeight+this.subtitleHeight-2,style)}style.weight=1.5;style.size*=1.5;
// Add title
if(options.title){Flotr.drawText(ctx,options.title,this.plotOffset.left+this.plotWidth/2,this.titleHeight-2,style)}style.weight=1.8;style.size*=.8;
// Add x axis title
...
// Add subtitle
if(options.subtitle)html.push('<div style="position:absolute;top:',this.titleHeight,"px;left:",this.plotOffset.left,"px;font-size:smaller;text-align:center;width:",this.plotWidth,'px;" class="flotr-subtitle">',options.subtitle,"</div>");html.push("</div>");html.push('<div class="flotr-axis-title" style="font-weight:bold;">');
// Add x axis title
...
// Utility
noConflict:function(root){var previous=root.envision;return function(){root.envision=previous;return this}}(this)};
// Visualization Class
(function(){var CN_FIRST="envision-first",CN_LAST="envision-last",T_VISUALIZATION='<div class="envision-visualization"></div>';T_COMPONENT_CONTAINER='<div class="envision-component-container"></div>';/**
 ...
 * @class
 */
function Visualization(options){this.options=options||{};this.components=[];this.node=null;this.rendered=false}Visualization.prototype={/**
...
render:function(element){var options=this.options;element=element||options.element;if(!element)throw"No element to render within.";this.node=bonzo.create(T_VISUALIZATION)[0];bonzo(this.node).addClass(options.name||"");this.container=element;bonzo(element).append(this.node);bonzo(element).data("envision",this);_.each(this.components,function(component){this._renderComponent(component)},this);this._updateClasses();this.rendered=true;return this},/**
   ...
   */
add:function(component){this.components.push(component);if(this.rendered){this._renderComponent(component);this._updateClasses()}return this},/**
   ...
   */
remove:function(component){var components=this.components,index=this.indexOf(component);if(index!==-1){components.splice(index,1);bonzo(component.container).remove();this._updateClasses()}return this},/**
...
setPosition:function(component,newIndex){var components=this.components;if(newIndex>=0&&newIndex<components.length&&this.remove(component)){if(this.rendered){if(newIndex===components.length)this.node.appendChild(component.container);else this.node.insertBefore(component.container,components[newIndex].container)}components.splice(newIndex,0,component);this._updateClasses()}return this},/**
   ...
   * @param {envision.Component} component
   */
indexOf:function(component){return _.indexOf(this.components,component)},/**
   * Gets the component at a position.
   *
   * @param {envision.Component} component
   * @returns {envision.Component}  The found component.
   */
getComponent:function(index){var components=this.components;if(index<components.length)return components[index]},/**
   ...
   * @returns {Boolean}
   */
isFirst:function(component){return this.indexOf(component)===0?true:false},/**
 ...
 * @class
 */
function Component(options){options=options||{};var node=bonzo.create(T_COMPONENT)[0];this.options=options;this.node=node;
// Instantiate Adapter
if(options.adapter){this.api=options.adapter}else if(options.adapterConstructor){this.api=new options.adapterConstructor(options.config)}else if(options.adapterCallback){this.api=options.adapterCallback.call(null,options.config)}else if(options.config){this.api=new V.adapters.flotr.Child(options.config||{})}
// this.id = _.uniqueId(CN_COMPONENT);
this.preprocessors=[]}Component.prototype={/**
   ...
   * @param {Element} [element]
   */
render:function(element){var node=this.node,options=this.options;element=element||options.element;if(!element)throw"No element to render within.";bonzo(element).addClass(options.name||"").append(this.node);this._setDimension("width");this._setDimension("height");this.container=element;this.draw(options.data,options.config)},/**
   * Draw the component.
   *
   * @param {Array} [data] Data for the adapter.
   * @param {Object} [options] Configuration object for the adapters draw method.
   */
draw:function(data,config){var api=this.api,options=this.options,preprocessors=this.preprocessors,clientData;clientData=data=data||options.data;config=config||options.config;if(!options.skipPreprocess&&data){clientData=[];_.each(api.getDataArray(data),function(d,index){var preprocessor=preprocessors[index]||new V.Preprocessor,isArray=_.isArray(d),isFunction=_.isFunction(d),unprocessed=isArray?d:isFunction?d:d.data,processData=options.processData,range=api.range(config),min=range.min,max=range.max,resolution=this.node.clientWidth,dataArray=d,processed,objectData;
// For object data
if(!isFunction&&!isArray){dataArray=d.data;objectData=_.extend({},d)}
// Do data function preprocessing
if(isFunction){processed=data(min,max,resolution)}else{
// Update if new data
if(dataArray!==preprocessor.data){preprocessor.setData(dataArray)}else{preprocessor.reset()}
// Do custom callback preprocessing
if(processData){processData.apply(this,[{preprocessor:preprocessor,min:min,max:max,resolution:resolution}]);processed=preprocessor.getData()}else{processed=preprocessor.bound(min,max).subsampleMinMax(resolution).getData()}}
// If present, transform the data for the API
if(api.transformData){processed=api.transformData(processed)}
// Object Data
if(objectData){objectData.data=processed;clientData.push(objectData)}else{clientData.push(processed)}},this)}if(api)api.draw(clientData,config,this.node)},/**
   * Trigger an event on the component's API.
   *
   * Arguments are passed through to the API.
   */
trigger:function(){this.api.trigger.apply(this.api,Array.prototype.concat.apply([this],arguments))},/**
   * Attach to an event on the component's API.
   *
   * Arguments are passed through to the API.
   */
attach:function(){this.api.attach.apply(this.api,Array.prototype.concat.apply([this],arguments))},/**
   * Detach a listener from an event on the component's API.
   *
   * Arguments are passed through to the API.
   */
detach:function(){this.api.detach.apply(this.api,Array.prototype.concat.apply([this],arguments))},/**
   * Destroy the component.
   *
   * Empties the container and calls the destroy method on the
   * component's API.
   */
destroy:function(){if(this.api&&this.api.destroy)this.api.destroy();bonzo(this.container).empty()},_setDimension:function(attribute){var node=this.node,options=this.options;if(options[attribute]){bonzo(node).css(attribute,options[attribute])}else{
//options[attribute] = parseInt(bonzo(node).css(attribute), 10);
options[attribute]=node.clientWidth}this[attribute]=options[attribute]}};V.Component=Component})();
// Interaction Class
(function(){var H=envision;/**
 ...
 * @class
 */
function Interaction(options){this.options=options=options||{};this.actions=[];this.actionOptions=[];this.followers=[];this.leaders=[];this.prevent={};if(options.leader){this.leader(options.leader)}}Interaction.prototype={/**
   * Add a component as an interaction leader.
   *
   * @param {envision.Component} component
   */
leader:function(component){this.leaders.push(component);_.each(this.actions,function(action,i){this._bindLeader(component,action,this.actionOptions[i])},this);return this},/**
   * Add a component as an interaction leader.
   *
   * @param {envision.Component} component
   */
follower:function(component){this.followers.push(component);return this},/**
   * Adds an array of components as both followers and leaders.
   *
   * @param {Array} components  An array of components
   */
group:function(components){if(!_.isArray(components))components=[components];_.each(components,function(component){this.leader(component);this.follower(component)},this);return this},/**
   ...
   * @param {Object} [options]
   */
add:function(action,options){this.actions.push(action);this.actionOptions.push(options);_.each(this.leaders,function(leader){this._bindLeader(leader,action,options)},this);return this},_bindLeader:function(leader,action,options){_.each(action.events,function(e){var handler=e.handler||e,consumer=e.consumer||e;leader.attach(handler,_.bind(function(leader,result){if(this.prevent[name])return;
// Apply custom callback configured for this action
if(options&&options.callback){options.callback.call(this,result)}this.prevent[name]=true;// Prevent recursions for this name
try{_.each(this.followers,function(follower){if(leader===follower)return;// Skip leader (recursion)
follower.trigger(consumer,result)},this)}catch(e){this.prevent[name]=false;throw e}this.prevent[name]=false},this))},this)}};H.Interaction=Interaction})();
// Preprocessor Class
(function(){/**
 ...
 * @class
 */
function Preprocessor(options){options=options||{};/**
   * Returns data.
   */
this.getData=function(){if(this.bounded)bound(this);return this.processing};this.reset=function(){this.processing=this.data;return this};/**
   * Set the data object.
   */
this.setData=function(data){var i,length;if(!_.isArray(data))throw new Error("Array expected.");if(data.length<2)throw new Error("Data must contain at least two dimensions.");length=data[0].length;for(i=data.length;i--;){if(!_.isArray(data[i]))throw new Error("Data dimensions must be arrays.");if(data[i].length!==length)throw new Error("Data dimensions must contain the same number of points.")}this.processing=data;this.data=data;return this};if(options.data)this.setData(options.data)}function getStartIndex(data,min){var i=_.sortedIndex(data,min);
// Include point outside range when not exact match
if(data[i]>min&&i>0)i--;return i}function getEndIndex(data,max){return _.sortedIndex(data,max)}function bound(that){delete that.bounded;var data=that.processing,length=that.length(),x=data[0],y=data[1],min=that.min||0,max=that.max||length,start=getStartIndex(x,min),end=getEndIndex(x,max);that.processing=[x.slice(start,end+1),y.slice(start,end+1)];that.start=start;that.end=end}Preprocessor.prototype={/**
   * Returns the length of the data set.
   *
   * @return {Number} Length of the data set.
   */
length:function(){return this.getData()[0].length},/**
   * Bounds the data set at within a range.
   *
   * @param {Number} min
   * @param {Number} max
   */
bound:function(min,max){if(!_.isNumber(min)||!_.isNumber(max))return this;this.min=min;this.max=max;this.bounded=true;return this},/**
...
fillOpacity:.4},/**
   ...
   * @param {Object} options
   */
draw:function(options){var context=options.context,lineWidth=options.lineWidth,shadowSize=options.shadowSize,offset;context.save();context.lineCap="butt";context.lineWidth=lineWidth;context.strokeStyle=options.color;this.plot(options);context.restore()},plot:function(options){var context=options.context,xScale=options.xScale,yScale=options.yScale,data=options.data,length=data.length-1,zero=yScale(0),x0,y0;if(length<1)return;x0=xScale(data[0][0]);y0=yScale(data[0][1]);context.beginPath();context.moveTo(x0,y0);for(i=0;i<length;++i){context.lineTo(xScale(data[i+1][0]),yScale(data[i+1][1]))}if(!options.fill||options.fill&&!options.fillBorder)context.stroke();if(options.fill){x0=xScale(data[0][0]);context.fillStyle=options.fillStyle;context.lineTo(xScale(data[length][0]),zero);context.lineTo(x0,zero);context.lineTo(x0,yScale(data[0][1]));context.fill();if(options.fillBorder){context.stroke()}}},extendYRange:function(axis,data,options,lines){var o=axis.options;
// HACK
if(!o.max&&o.max!==0||!o.min&&o.min!==0){axis.max+=options.lineWidth*.01;axis.min-=options.lineWidth*.01}}});/** Bars **/
Flotr.addType("whiskers",{options:{show:false,// => setting to true will show bars, false will hide
lineWidth:2,// => in pixels
barWidth:1,// => in units of the x axis
fill:true,// => true to fill the area from the line to the x axis, false for (transparent) no fill
fillColor:null,// => fill color
...
// Left
if(min<=1){context.moveTo(0,height);context.lineTo(0,-.5)}else{context.moveTo(min,height);context.quadraticCurveTo(min,half,Math.max(min-half,min/2),half);context.lineTo(Math.min(half,min/2),half);context.quadraticCurveTo(0,half,.5,-.5)}
// Top
context.lineTo(width-.5,-.5);
// Right
if(max>=width-1){context.lineTo(max,height)}else{context.quadraticCurveTo(width,half,Math.max(width-half,width-(width-max)/2),half);context.lineTo(Math.min(max+half,width-(width-max)/2),half);context.quadraticCurveTo(max,half,max,height)}context.stroke();context.closePath();context.fill();context.restore()}},trigger:function(component,name,options){if(name==="zoom"){this.zoom(component,options)}else if(name==="reset"){this.reset(component)}},zoom:function(component,options){var x=options.x||{},min=x.min,max=x.max,api=component.api;component.draw(null,{min:min,max:max})},reset:function(component){component.draw(null,{min:component.width/2,max:component.width/2})}};envision.components.QuadraticDrawing=QuadraticDrawing})();/**
 * Templates namespace.
 *
...
$.getJSON(DATA_DIR+current_datsel.properties,function(data){current_datsel.props=data;console.log('~~ Member properties of "'+current_datsel.id+'" have been loaded')})}if(current_datsel.datasets[dsi].data!==undefined){current_setsel=current_datsel.datasets[dsi];genChart();genGrid()}else{
// loading feedback
var l=0;var lAnim=setInterval(function(){var txt=["loading... &nbsp; &nbsp; ┬──┬","loading... &nbsp; &nbsp; ┬──┬","loading... (°o°） ┬──┬","loading... &nbsp;(°o°）┬──┬","loading... (╯°□°）╯ ┻━┻","loading... (╯°□°）╯︵ ┻━┻","loading... (╯°□°）╯︵ ︵ ┻━┻","loading... (╯°□°）╯︵ ︵ ︵ ┻━┻","loading... ︵ ︵ ︵ ┻━┻","loading... ︵ ︵ ┻━┻","loading... ︵ ┻━┻","loading... &nbsp; &nbsp; ┻━┻ &nbsp; &nbsp; (ツ)","loading... &nbsp; &nbsp; ┻━┻ &nbsp; &nbsp;(ツ)","loading... &nbsp; &nbsp; ┻━┻ &nbsp; (ツ)","loading... &nbsp; &nbsp; ┻━┻ &nbsp;(ツ)","loading... &nbsp; &nbsp; ┬──┬ ¯\\_(ツ)","loading... &nbsp; &nbsp; ┬──┬ (ツ)"];$("#legend").html(txt[l]);l=(l+1)%17},180);var lBM=new Date;console.log("~~ starting to load dataset "+current_datsel.datasets[dsi].strings.label+" ~~ ");$.getJSON(DATA_DIR+gdata[0].datasets[dsi].file,function(data){console.log(" |BM| finished loading "+current_datsel.datasets[dsi].strings.label+" data (took "+(new Date-lBM)+"ms)");clearInterval(lAnim);current_datsel.datasets[dsi].data=data;current_setsel=current_datsel.datasets[dsi];genChart();genGrid()})}}////////////////////
/// global scope ///
////////////////////
////////////////////
/// pseudo constants
...
$(function(){
// init stuff
lastTransformState={scale:1,translate:[0,0]};/*Highcharts.setOptions({
		global: {
			useUTC: false
		}
	});*/
$("#zoom-slider").attr("min",M_ZOOM_RANGE[0]).attr("max",M_ZOOM_RANGE[1]);$("#freezer>input").on("change",function(){allow_redraw=!this.checked;if(this.checked){$("#legend").css("opacity",".5")}else{$("#legend").css("opacity","1");genGrid()}});$("#colorizer>input").on("change",function(){colorize=!this.checked;genGrid()});$("#infolist").on("scroll",infolistScroll);
// bind window resize handling
$(window).resize(function(){clearTimeout(resizeTimer);resizeTimer=setTimeout(onResize,400)});
// zoombehaviour
...
$(document).on("meta_files_ready",function(){current_datsel=gdata[0];// TODO [get from dom] (depends on data management)
if(!statifyUrl()){$("#filter input")[DEFAULT_DATASET].click()}});
/// TODO
// load all the meta data meta files
var bmMETA=new Date;console.log("~~ started loading the meta files (total of "+META_FILES.length+") ~~ ");var mfc=0;// meta file counter
for(var i=0;i<META_FILES.length;i++){$.getJSON(DATA_DIR+META_FILES[i],function(data){gdata[mfc]=data;// TODO
...
clearTimeout(redrawTimer);redrawTimer=setTimeout(function(){generateGrid(reso,mAE,data)},200)}/**
* Generate data grid for the map
* parameters optional */
function generateGrid(reso,mAE,data){
/// TODO poperly use global tilemap 
if(!allow_redraw){return false}if(data===undefined){data=current_setsel}// todo dynamic from filter?
if(mAE===undefined){mAE=getBounds()}if(reso===undefined){reso=calcReso()}currentGenGrid++;var thisGenGrid=currentGenGrid;$("#legend").html("<em>massive calculations...</em>");setTimeout(function(){//timeout for dom redraw
console.log("/~~ generating new grid with resolution "+reso+" ~~\\");var bms=new Date;console.log("  ~ start iterating data");var count=0;cellmap={};// reset cellmap
...
if(mAE[0].min<C_WMIN){mAE[0].min=C_WMIN}if(mAE[0].max>C_WMAX){mAE[0].max=C_WMAX}if(mAE[1].min<C_HMIN){mAE[1].min=C_HMIN}if(mAE[1].max>C_HMAX){mAE[1].max=C_HMAX}var tMin=0;while(mAE[0].min>C_WMIN+(tMin+1)*data.parent.tile_width){tMin++}var tMax=tMin;while(mAE[0].max>C_WMIN+(tMax+1)*data.parent.tile_width){tMax++}console.log("  # will iterate over tiles "+tMin+" to "+tMax);var finish=function(){console.log("  |BM| iteration complete ("+(new Date-bms)+"ms)");drawPlot(true,cellmap,reso);console.log("  |BM| finished genGrid (total of "+(new Date-bms)+"ms)");$("#legend").html("<em>inside the visible area</em><br>"+
...
l=data.data[i][j].length;for(var k=0;k<l;k++){// go over each event in key and
a=data.data[i][j][k];if(section_filter(a,mAE)){// if it actually lies within map bounds
ti=coord2index(a[ARR_M_LON],a[ARR_M_LAT],reso);if(cellmap[ti]===undefined){cellmap[ti]=[]}// aggregate it on the grid
cellmap[ti].push([a[ARR_M_I],j]);cellmapprog[ti]=cellmap[ti];count++}}}}
// draw each tile after aggregating
drawPlot(false,cellmapprog,reso);setTimeout(function(){iterate(++offset)},1)}else{// iterated over all tiles
finish()}};iterate(0)},1)}
///////////////
/// filters ///
///////////////
// true if object geo lies within currently visible section
function section_filter(obj,aE){return obj[ARR_M_LAT]>=aE[1].min&&obj[ARR_M_LAT]<=aE[1].max&&(obj[ARR_M_LON]>=aE[0].min&&obj[ARR_M_LON]<=aE[0].max)}
//////////////////
/// aggregators //
/*/////////////////
...
var rmax,gmax,bmax,rlog_factorm,glog_factor,blog_factor;if(colorize){rmax=current_setsel.colorScale.min[0];rlog_factor=(rmax-current_setsel.colorScale.max[0])/Math.log(drawdat.max);gmax=current_setsel.colorScale.min[1];glog_factor=(gmax-current_setsel.colorScale.max[1])/Math.log(drawdat.max);bmax=current_setsel.colorScale.min[2];blog_factor=(bmax-current_setsel.colorScale.max[2])/Math.log(drawdat.max)}else{bmax=255;//215;
blog_factor=bmax/drawdat.max;//Math.log(drawdat.max);
gmax=235;//205;
glog_factor=gmax/Math.log(drawdat.max);rmax=185;//14;
rlog_factor=rmax/Math.log(drawdat.max)}var canvasRenderBM=new Date;
// sizes and radii of primitiva
var bleed=1+1/lastTransformState.scale*.25;// 1.25; // larger size for bleeding with alpha channel
...
//fc = 
"rgba("+Math.floor(rmax-Math.floor(Math.log(d[1])*rlog_factor))+","+Math.floor(gmax-Math.floor(Math.log(d[1])*glog_factor))+","+Math.floor(bmax-Math.floor(d[1]*blog_factor))+","+".85)";//((d[1]/drawdat.max)/4+0.6)+")";
...
function canvasCoord2geoCoord(x,y){var t=lastTransformState;return{x:-180+(-t.translate[0]+x)/(canvasW*t.scale)*360,y:90-(-t.translate[1]+y)/(canvasH*t.scale)*180}}/**
* map tooltip */
function canvasMouseMove(){if(drawdat===undefined){return false}// no drawing, no tooltip!
var x=d3.event.pageX-canvasL;var y=d3.event.pageY-canvasT;
...
clearTimeout(bubbleTimer);$("div#bubble").css("opacity","1").css("bottom",viewportH-d3.event.pageY+M_BUBBLE_OFFSET+"px").css("right",viewportW-d3.event.pageX+M_BUBBLE_OFFSET+"px").html(cell.length+" <em>"+current_setsel.strings.label+"</em><br>"+"<span>["+gc.x.toFixed(2)+", "+gc.y.toFixed(2)+"]</span>")}else{
// hide the info bubble
clearTimeout(bubbleTimer);bubbleTimer=setTimeout(function(){$("div#bubble").css("opacity","0")},250)}}function canvasMouseClick(){
...
function infolistScrollFkt(){var cellinfo=$("#cellinfo");var infolist=$("#infolist");var infolistT=cellinfo.position().top+infolist.position().top;var infolistB=infolistT+infolist.height();var qarray=[];infolist.find("a.q").each(function(){var t=this.getBoundingClientRect().top;if(t>=infolistT&&t<=infolistB){qarray.push(this)}});
//console.log(qarray);
...
/// timeline ///
////////////////
function genChart(data){if(data===undefined){data=current_setsel}// TODO
console.log("/~~ generating chart data ~~\\ ");var benchmark_chart=new Date;
////
...
var x=[],y=[],ticks=[];var dat_obj={},i,j,d;var tilecount=(C_WMAX-C_WMIN)/data.parent.tile_width;for(i=0;i<tilecount;i++){for(j=data.min;j<=data.max;j++){d=data.data[i][j];if(d!==undefined){if(dat_obj[j]===undefined){dat_obj[j]=d.length}else{dat_obj[j]+=d.length}}}}for(i=data.min;i<=data.max;i++){if(dat_obj[i]!==undefined){x.push(i);y.push(dat_obj[i])}}chartdat.push([x,y]);console.log("  |BM| iterating and sorting finished (took "+(new Date-benchmark_chart)+"ms)");initChart();console.log("  |BM| chart creation complete (total of "+(new Date-benchmark_chart)+"ms");console.log("\\~~ finished generating chart ~~/ ")}function initChart(){if(chartdat.length<=0){return false}if(chart!==undefined){chart.destroy()}var connectionH=10;// height of connection component
...
var range=getTimeSelection();$("#range-tt-min").text(range.min);$("#range-tt-max").text(range.max);genGrid()};var detail,detailOptions,summary,summaryOptions,connection,connectionOptions;
// Configuration for detail (top view):
detailOptions={name:"detail",data:chartdat,height:detailH,width:containerW,title:"Timeline",
// Flotr Configuration
config:{bars:{lineWidth:1,show:true,fill:true,fillOpacity:.6},mouse:{track:true,trackY:false,trackAll:true,sensibility:1,trackDecimals:4,position:"ne",lineColor:"#ff9900",fillColor:"#ff9900",fillOpacity:.6,trackFormatter:function(o){return"<em>"+parseInt(o.y)+" "+current_setsel.strings.label+"</em> in "+parseInt(o.x)}},yaxis:{autoscale:true,autoscaleMargin:.05,noTicks:4,showLabels:true,min:0}}};
// Configuration for summary (bottom view):
summaryOptions={name:"summary",data:chartdat,height:summaryH,width:containerW,
// Flotr Configuration
config:{lines:{show:true,lineWidth:1,fill:true,fillOpacity:.2,fillBorder:true},xaxis:{noTicks:5,showLabels:true},yaxis:{autoscale:true,autoscaleMargin:.1},handles:{show:true},selection:{mode:"x"}}};connectionOptions={name:"connection",adapterConstructor:envision.components.QuadraticDrawing,height:connectionH,width:containerW};
// Building the vis:
chart=new envision.Visualization;detail=new envision.Component(detailOptions);summary=new envision.Component(summaryOptions);connection=new envision.Component(connectionOptions);interaction=new envision.Interaction;
// Render Visualization
chart.add(detail).add(connection).add(summary).render(container.get(0));
// Wireup Interaction
interaction.leader(summary).follower(detail).follower(connection).add(envision.actions.selection,{callback:selCallback});appendTimelineRangeTips();
// set to initial selection state
summary.trigger("select",initSelection)}/**
* returns sanitized selection of the timeline */
function getTimeSelection(){var cAE,min=current_setsel.min,max=current_setsel.max,sel=chart.components[2].api.flotr.selection;if(sel.selecting!==false){cAE=sel.getArea();cAE.min=parseInt(cAE.x1);cAE.max=parseInt(cAE.x2)}else{cAE={min:min,max:max}}
// TODO simplify
