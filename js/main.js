...
var map={},forAll=function(element,type,original,handler,fn){if(!type||type==="*"){// search the whole registry
...
function Color(r,g,b,a){this.rgba=["r","g","b","a"];var x=4;while(-1<--x){this[this.rgba[x]]=arguments[x]||(x==3?1:0)}this.normalize()}// Constants
...
empty:function(element){element.innerHTML=""},hide:function(element){Flotr.DOM.setStyles(element,{display:"none"})},/**
   ...
   */
insert:function(element,child){if(_.isString(child))element.innerHTML+=child;else if(_.isElement(child))element.appendChild(child)},// @TODO find xbrowser implementation
...
if(data[i][1]===null||data[i+1][1]===null){if(options.fill){if(i>0&&data[i][1]){context.stroke();fill();start=null;context.closePath();context.beginPath()}}continue}// Zero is infinity for log scales
...
_negative:[]},draw:function(options){var context=options.context;this.current+=1;context.save();context.lineJoin="miter";// @TODO linewidth not interpreted the right way.
...
if(datum>0){positiveSums[value]=(positiveSums[value]||0)+datum;newmax=Math.max(newmax,positiveSums[value])}else{negativeSums[value]=(negativeSums[value]||0)+datum;newmin=Math.min(newmin,negativeSums[value])}}}// End of bars
 ...
 */
(function(){function isLeftClick(e,type){return e.which?e.which===1:e.button===0||e.button===1}function boundX(x,graph){//return Math.min(Math.max(0, x), graph.plotWidth - 1);
...
mouseTrack.style.left=left+n.xaxis.d2p(n.x)-D.size(mouseTrack).width/2+"px"}else if(!/[ns]/.test(p)){// Center Vertically
mouseTrack.style.top=top+n.yaxis.d2p(n.y)-D.size(mouseTrack).height/2+"px"}}}})})();(function(){var D=Flotr.DOM;Flotr.addPlugin("labels",{callbacks:{"flotr:afterdraw":function(){this.labels.draw()}},draw:function(){// Construct fixed width label boxes, which can be styled easily.
...
if(options.title){Flotr.drawText(ctx,options.title,this.plotOffset.left+this.plotWidth/2,this.titleHeight-2,style)}style.weight=1.8;style.size*=.8;// Add x axis title
 ...
 * @namespace
 */
var envision={// Globals
_:Flotr._,// Underscore.js, functional microlib
bean:Flotr.bean,// Bean, events
bonzo:bonzo,// Bonzo, dom
// Utility
noConflict:function(root){var previous=root.envision;return function(){root.envision=previous;return this}}(this)};// Visualization Class
 ...
 * @class
 */
function Component(options){options=options||{};var node=bonzo.create(T_COMPONENT)[0];this.options=options;this.node=node;// Instantiate Adapter
if(options.adapter){this.api=options.adapter}else if(options.adapterConstructor){this.api=new options.adapterConstructor(options.config)}else if(options.adapterCallback){this.api=options.adapterCallback.call(null,options.config)}else if(options.config){this.api=new V.adapters.flotr.Child(options.config||{})}// this.id = _.uniqueId(CN_COMPONENT);
...
render:function(element){var node=this.node,options=this.options;element=element||options.element;if(!element)throw"No element to render within.";bonzo(element).addClass(options.name||"").append(this.node);this._setDimension("width");this._setDimension("height");this.container=element;this.draw(options.data,options.config)},/**
   ...
   */
draw:function(data,config){var api=this.api,options=this.options,preprocessors=this.preprocessors,clientData;clientData=data=data||options.data;config=config||options.config;if(!options.skipPreprocess&&data){clientData=[];_.each(api.getDataArray(data),function(d,index){var preprocessor=preprocessors[index]||new V.Preprocessor,isArray=_.isArray(d),isFunction=_.isFunction(d),unprocessed=isArray?d:isFunction?d:d.data,processData=options.processData,range=api.range(config),min=range.min,max=range.max,resolution=this.node.clientWidth,dataArray=d,processed,objectData;// For object data
if(!isFunction&&!isArray){dataArray=d.data;objectData=_.extend({},d)}// Do data function preprocessing
if(isFunction){processed=data(min,max,resolution)}else{// Update if new data
if(dataArray!==preprocessor.data){preprocessor.setData(dataArray)}else{preprocessor.reset()}// Do custom callback preprocessing
if(processData){processData.apply(this,[{preprocessor:preprocessor,min:min,max:max,resolution:resolution}]);processed=preprocessor.getData()}else{processed=preprocessor.bound(min,max).subsampleMinMax(resolution).getData()}}// If present, transform the data for the API
if(api.transformData){processed=api.transformData(processed)}// Object Data
...
detach:function(){this.api.detach.apply(this.api,Array.prototype.concat.apply([this],arguments))},/**
   ...
   * component's API.
   */
destroy:function(){if(this.api&&this.api.destroy)this.api.destroy();bonzo(this.container).empty()},_setDimension:function(attribute){var node=this.node,options=this.options;if(options[attribute]){bonzo(node).css(attribute,options[attribute])}else{//options[attribute] = parseInt(bonzo(node).css(attribute), 10);
options[attribute]=node.clientWidth}this[attribute]=options[attribute]}};V.Component=Component})();// Interaction Class
...
follower.trigger(consumer,result)},this)}catch(e){this.prevent[name]=false;throw e}this.prevent[name]=false},this))},this)}};H.Interaction=Interaction})();// Preprocessor Class
(function(){/**
...
newX.push(x[end]);newY.push(y[end]);this.processing=[newX,newY];this.start=start;this.end=end}else{this.bounded=bounded}return this},/**
   ...
   * @param {Number} resolution
   */
subsample:function(resolution){var bounded=this.bounded;delete this.bounded;var data=this.processing,length=this.length(),x=data[0],y=data[1],start=bounded?getStartIndex(x,this.min):0,end=bounded?getEndIndex(x,this.max):length-1,unit=(end-start+1)/resolution,newX=[],newY=[],i,index;if(end-start+1>resolution){// First
newX.push(x[start]);newY.push(y[start]);for(i=1;i<resolution;i++){if(i*unit>=end-unit)break;index=Math.round(i*unit)+start;newX.push(x[index]);newY.push(y[index])}// Last
 ...
 */
envision.actions=envision.actions||{};envision.actions.hit={events:["hit","mouseout"]};envision.actions.selection={events:[{handler:"select",consumer:"zoom"},// Reset on click, avoids re-drawing the leader.
{handler:"click",consumer:"reset"}]};envision.actions.zoom={events:[// Zoom on the followers as selecting
{handler:"select",consumer:"zoom"},// Zoom on the leader after mouseup
"zoom",// Reset all on click
"reset"]};/**
 ...
 */
envision.templates=envision.templates||{};(function(){var V=envision;// Custom data processor
...
return value};if(options.xTickFormatter){defaults.summary.config.xaxis.tickFormatter=options.xTickFormatter}defaults.price.config.yaxis.tickFormatter=options.yTickFormatter||function(n){return"$"+n};price=new V.Component(defaults.price);volume=new V.Component(defaults.volume);connection=new V.Component(defaults.connection);summary=new V.Component(defaults.summary);// Render visualization
vis.add(price).add(volume).add(connection).add(summary).render(options.container);// Define the selection zooming interaction
selection.follower(price).follower(volume).follower(connection).leader(summary).add(V.actions.selection,options.selectionCallback?{callback:options.selectionCallback}:null);// Define the mouseover hit interaction
hit.group([price,volume]).add(V.actions.hit);// Optional initial selection
if(options.selection){summary.trigger("select",options.selection)}// Members
this.vis=vis;this.selection=selection;this.hit=hit;this.price=price;this.volume=volume;this.summary=summary}V.templates.Finance=Finance})();(function(){var V=envision;function getDefaults(){return{detail:{name:"envision-timeseries-detail",config:{"lite-lines":{lineWidth:1,show:true}}},summary:{name:"envision-timeseries-summary",config:{"lite-lines":{lineWidth:1,show:true},handles:{show:true},selection:{mode:"x"},yaxis:{autoscale:true,autoscaleMargin:.1}}},connection:{name:"envision-timeseries-connection",adapterConstructor:V.components.QuadraticDrawing}}}function TimeSeries(options){var data=options.data,defaults=getDefaults(),vis=new V.Visualization({name:"envision-timeseries"}),selection=new V.Interaction,detail,summary,connection;// Fill Defaults
if(options.defaults){defaults=Flotr.merge(options.defaults,defaults)}defaults.detail.data=data.detail;defaults.summary.data=data.summary;// Build Components
detail=new V.Component(defaults.detail);connection=new V.Component(defaults.connection);summary=new V.Component(defaults.summary);// Render visualization
vis.add(detail).add(connection).add(summary).render(options.container);// Selection action
selection.follower(detail).follower(connection).leader(summary).add(V.actions.selection,options.selectionCallback?{callback:options.selectionCallback}:null);// Optional initial selection
...
if(current_datsel.props===undefined){// TODO ? more loading feedback
$.getJSON(DATA_DIR+current_datsel.properties,function(data){current_datsel.props=data;console.log('~~ Member properties of "'+current_datsel.id+'" have been loaded')})}if(current_datsel.datasets[dsi].data!==undefined){current_setsel=current_datsel.datasets[dsi];genChart();genGrid()}else{// loading feedback
...
var TPI=Math.PI*2;///_________________
/// pseudo constants
////////////////////
///////////////
/// global vars
...
if(!statifyUrl()){$("#filter input")[DEFAULT_DATASET].click()}});/// TODO
...
* parameters optional */
function generateGrid(reso,mAE,data){/// TODO poperly use global tilemap 
...
cellmap[ti].push([a[ARR_M_I],j]);cellmapprog[ti]=cellmap[ti];count++}}}}// draw each tile after aggregating
drawPlot(i,cellmapprog,reso);setTimeout(function(){iterate(++offset)},1)}else{// iterated over all tiles
finish()}};iterate(0)},1)}///////////////
/// filters ///
///////////////
...
var p=index2canvasCoord(i);p=canvasCoord2geoCoord(p[0]+drawdat.rx,p[1]+drawdat.ry);var x=p.x.toFixed(2),y=p.y.toFixed(2);highlightCell(i,true);//console.log(cell);
...
if(timeSel===undefined){timeSel={data:{// TODO this could go into dataset config options
x:{min:1500,max:2014}}}}var selCallback=function(){// callback function for selection change
var range=getTimeSelection();timeSel.data.x={min:range.min,max:range.max};$("#range-tt-min").text(range.min);$("#range-tt-max").text(range.max);genGrid()};var detail,detailOptions,summaryOptions,// summary, (global)
connection,connectionOptions;// Configuration for detail (top view):
detailOptions={name:"detail",data:chartdat,height:detailH,width:containerW,title:"Timeline",// Flotr Configuration
config:{bars:{lineWidth:1,show:true,fill:true,fillOpacity:.6},mouse:{track:true,trackY:false,trackAll:true,sensibility:1,trackDecimals:4,position:"nw",lineColor:"#ff9900",fillColor:"#ff9900",fillOpacity:.6,trackFormatter:function(o){return"<em>"+current_setsel.strings.label+"</em> in "+parseInt(o.x)+": <em>"+parseInt(o.y)+"</em>"}},yaxis:{autoscale:true,autoscaleMargin:.05,noTicks:4,showLabels:true,min:0}}};// Configuration for summary (bottom view):
summaryOptions={name:"summary",data:chartdat,height:summaryH,width:containerW,// Flotr Configuration
config:{lines:{show:true,lineWidth:1,fill:true,fillOpacity:.2,fillBorder:true},xaxis:{noTicks:5,showLabels:true},yaxis:{autoscale:true,autoscaleMargin:.1},handles:{show:true},selection:{mode:"x"}}};connectionOptions={name:"connection",adapterConstructor:envision.components.QuadraticDrawing,height:connectionH,width:containerW};// Building the vis:
...
function changeTimeSel(min,max,relative){if(relative===undefined){relative=true}if(relative){min+=timeSel.data.x.min;max+=timeSel.data.x.max}timeSel={data:{x:{min:min,max:max}}}}/**
...
return{min:cAE.min>=min?cAE.min:min,max:cAE.max<=max?cAE.max:max}}function appendTimelineRangeTips(){var cont=$('<div id="range-tt-min" class="range-tt hover-tt"></div>'+'<div id="range-tt-max" class="range-tt hover-tt"></div>');//.hide();
$("#chart").append(cont);/*	$("#chart .detail").on("mouseenter", function() {
